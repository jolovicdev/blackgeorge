...
The session ID is auto-generated, or you can provide your own. If the ID does not exist yet, a new session is created with that ID:
...
1. Sends them with the new user message
1. Saves the updated conversation after each response
...
With `create=False`, `session()` returns `None` if the session does not exist or belongs to a different worker. The default `create=True` behavior opens an existing session or creates it when the ID is new.
...
Closing releases the session store connection and preserves the conversation. Sessions also support the context manager protocol.
...
1. Old messages are summarized automatically
1. The compacted messages are saved back to the session
1. Next turn loads the already-compacted messages
...
Both use the same underlying storage and produce identical results. If you already have a running event loop, use `arun()` instead of `run()`.

## Streaming responses

The `stream_run()` and `astream_run()` methods run with streaming enabled and yield events in real time as they are emitted for that run:
