...
- `model` (optional): Model to use (default: `openrouter/google/gemini-3-pro-image-preview`)
...
For models that generate images through chat completions, Blackgeorge automatically falls back to `completion(..., modalities=["image", "text"])` if the image endpoint returns no image data.
...
1. **Model Limits**: Check model documentation for maximum image/video duration limits
1. **Cost**: Multimodal requests typically cost more than text-only
1. **URLs vs Base64**: URLs are more efficient for remote files; use base64 for local files
1. **Error Handling**: Always validate that your chosen model supports the media type
