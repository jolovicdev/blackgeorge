...
A flow produces a report. If there are multiple steps, the content is combined with step headers. `Report.metrics` carries the run's `usage` and `cost_usd` totals summed across every step, and those totals survive a pause and resume.
...
`context.outputs` holds the completed reports so far. It grows as each step finishes, including steps nested inside `Condition`, `Router`, `Loop`, and `Parallel`, so job builders and predicates inside a composite see the results of the steps that ran before them.
