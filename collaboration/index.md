...
Both `Blackboard` and `Channel` are thread-safe. They use internal locks to ensure safe concurrent access from multiple workers. Mutable values, message content, and metadata are copied at the API boundary so callers cannot alter stored collaboration state without an explicit write.
